...
const TABS = [
  ...
  { href: "/teacher/documents", label: "Generate (AI)" },
...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    else if (user.role !== "teacher" && user.role !== "admin") router.replace("/student");
  }, [user, loading, router]);
    ...
    <div className="min-h-screen bg-paper">
      <div className="flex items-center justify-between px-5 py-3 border-b border-line bg-white sticky top-0 z-10">
        <div>
          ...
          <div className="text-[10.5px] text-inksoft uppercase tracking-wide">Teacher console</div>
        </div>
